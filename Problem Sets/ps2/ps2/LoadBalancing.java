/**
 * Contains static routines for solving the problem of balancing m jobs on p processors
 * with the constraint that each processor can only perform consecutive jobs.
 */
public class LoadBalancing {

    /**
     * Checks if it is possible to assign the specified jobs to the specified number of processors such that no
     * processor's load is higher than the specified query load.
     *
     * @param jobSizes the sizes of the jobs to be performed
     * @param queryLoad the maximum load allowed for any processor
     * @param p the number of processors
     * @return true iff it is possible to assign the jobs to p processors so that no processor has more than queryLoad load.
     */
    public static boolean isFeasibleLoad(int[] jobSizes, int queryLoad, int p) {
        boolean result = true;
        try { // not so familiar with the try catch implementation, got it from online to catch errors
            if (jobSizes.length == 0 || p == 0) {
                return false;
            } else {
                int sum = 0; // track each processor
                int counter = 0; // track number of processors that are "filled"
                for (int i = 0; i < jobSizes.length; i++) { // I can't think of a solution faster than O(n)
                    sum = sum + jobSizes[i];
                    if (counter >= p || jobSizes[i] > queryLoad) { // if the array require more processors than indicated, or if any single load is greater than queryload
                        result = false;
                        break;
                    } else if (i == jobSizes.length - 1) {
                        return result; // to prevent jobSizes[i + 1] from being run when i is at the end of array.
                        // Since no. of processors not exceeding and last value not greater than queryLoad is a pre-condition, default is always true
                    }
                    else if (sum + jobSizes[i + 1] > queryLoad) {
                        counter++;
                        sum = 0; // reset to track new processor
                    }
                }
            }
        } catch (Exception e) {
            result = false;
        }
        return result;
    }



    /**
     * Returns the minimum achievable load given the specified jobs and number of processors.
     *
     * @param jobSizes the sizes of the jobs to be performed
     * @param p the number of processors
     * @return the maximum load for a job assignment that minimizes the maximum load
     */
    public static int findLoad(int[] jobSizes, int p) {
        int sum = 0;
        int max = 0;
        if (jobSizes.length == 0 || p <= 0) {
            return -1;
        } else {
            for (int i = 0; i < jobSizes.length; i++) { // only run once
                sum = sum + jobSizes[i];
                max = Math.max(jobSizes[i], max);
            }
            int begin = max;
            int end = sum;
            int mid = 0;
            while (begin < end) {
                mid = (begin + end) / 2;
                System.out.println(mid);
                if (isFeasibleLoad(jobSizes, mid, p)) {
                    end = mid;
                } else {
                    begin = mid + 1;
                }
            }
            return begin;
        }
    }
    // O(nlog(nM)). Worst case sum of array is nM, helper isFeasibleLoad is O(n) and is run once every search
    // These are some arbitrary testcases.
    public static int[][] testCases = {
            {1, 3, 5, 7, 9, 11, 10, 8, 6, 4},
            {67, 65, 43, 42, 23, 17, 9, 100},
            {4, 100, 80, 15, 20, 25, 30},
            {2, 3, 4, 5, 6, 7, 8, 100, 99, 98, 97, 96, 95, 94, 93, 92, 91, 90, 89, 88, 87, 86, 85, 84, 83},
            {7}
    };

    /**
     * Some simple tests for the findLoad routine.
     */
    public static void main(String[] args) {
        for (int p = 1; p < 30; p++) {
            System.out.println("Processors: " + p);
            for (int[] testCase : testCases) {
                System.out.println(findLoad(testCase, p));
            }
        }
    }
}
