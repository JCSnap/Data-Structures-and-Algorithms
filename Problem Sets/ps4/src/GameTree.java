...
public class GameTree {
    ...
    int findValue() {
        ...
        root.value = WinValue(root.children, Player.ONE);
        return root.value;
    }

    private int WinValue(TreeNode[] children, Player player) {
        if (player == Player.ONE) {
            int max = Integer.MIN_VALUE;
            for (int i = 0; i < children.length; i ++) {
                TreeNode curChild = children[i];
                if (curChild.leaf) {
                    max = Math.max(curChild.value, max);
                } else { // current node contains children
                    int winChildren = WinValue(curChild.children, other(player));
                    curChild.value = winChildren;
                    max = Math.max(winChildren, max);
                }
            }
            return max;
        } else {
            ...
            for (int i = 0; i < children.length; i++) {
                TreeNode curChild = children[i];
                if (curChild.leaf) {
                    min = Math.min(curChild.value, min);
                } else {
                    int winChildren = WinValue(curChild.children, other(player));
                    curChild.value = winChildren;
                    min = Math.min(winChildren, min);
                }
            }
            return min;
        }
